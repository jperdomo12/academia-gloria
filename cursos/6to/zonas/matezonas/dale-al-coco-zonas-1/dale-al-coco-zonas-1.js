...
import {
  ...
  BLOQUES_RESULTADO,
  PRACTICA_COCO,
  FAMILIAS_PRUEBA_COCO
} from "./dale-al-coco-zonas-1-data.js";
...
function renderPractica() {
  detenerLecturaActual();
  const reto = PRACTICA_COCO[estado.practicaIndice];
  const porcentaje = ((estado.practicaIndice + 1) / PRACTICA_COCO.length) * 100;
  $("barraPractica").style.width = `${porcentaje}%`;
  $("estadoPractica").textContent = `Reto ${estado.practicaIndice + 1} de ${PRACTICA_COCO.length}`;
...
$("botonComprobar").addEventListener("click", () => {
  ...
  if (correcta) {
    ...
    $("botonSiguiente").textContent = estado.practicaIndice === PRACTICA_COCO.length - 1
      ? "Ir a la prueba →"
      : "Siguiente reto →";
  } else {
    botonElegido?.classList.add("incorrecta");
    $("mensajePractica").textContent = "Todavía no. Prueba otra estrategia o pide una pista.";
    $("mensajePractica").className = "message revisar";
  }
});

$("botonSiguiente").addEventListener("click", () => {
  if (estado.practicaIndice < PRACTICA_COCO.length - 1) {
  ...
  if (modo === MODOS_SESION_ACADEMICA.APRENDIZAJE) {
    try {
      recientes = await obtenerVariantesRecientes({
        actividadId:COCO_META.actividadId,
        maximoSesiones:3
      });
    } catch (error) {
      console.warn("No se pudo consultar el historial de variantes de Dale al coco.", error);
    }
  }

  estado.preguntasPrueba = FAMILIAS_PRUEBA_COCO.map(familia => {
...
async function iniciarPrueba() {
  ...
  estado.tiempo.reiniciar("prueba-dale-al-coco-zonas-1");
...
function renderPreguntaPrueba() {
  ...
  $("bloquePrueba").textContent = bloqueResultado(pregunta.bloqueId)?.titulo || "Matezonas";
...
function construirMapaResultados() {
  return BLOQUES_RESULTADO.map(bloque => {
    ...
    let textoEstado = "Vuelve a la estrategia de este bloque y prueba otra variante.";
...
async function finalizarPrueba() {
  ...
  $("resultadoPruebaResumen").innerHTML = `
    ...
    <p>Este resultado describe esta sesión de práctica. No es una calificación escolar.</p>
  `;
  ...
  if (reforzar.length) {
    $("textoCierre").textContent = `${nombreAlumno}, ya tenemos una fotografía útil: algunas estrategias están contigo y otras conviene volver a practicar con calma.`;
  } else if (enCamino.length) {
    $("textoCierre").textContent = `${nombreAlumno}, las estrategias principales están apareciendo. Unas variantes más ayudarán a hacerlas más firmes.`;
  } else {
    $("textoCierre").textContent = `${nombreAlumno}, hoy las cuatro formas de pensar de este material se vieron firmes.`;
  }
  ...
  try {
    estado.guardado = await guardarSesionAcademica({
      modo,
      actividadId:COCO_META.actividadId,
      tituloActividad:COCO_META.tituloActividad,
      versionActividad:COCO_META.version,
      cursoReferencia:COCO_META.cursoReferencia,
      materia:COCO_META.materia,
      tema:COCO_META.tema,
      origen:"curso",
    ...
    console.error("No se pudo guardar la sesión de Dale al coco.", error);
  ...
  if (typeof mostrarCelebracion === "function") {
    mostrarCelebracion({
      titulo:"¡Reto recorrido!",
      mensaje:"Ya sabemos qué estrategias de Dale al coco seguir fortaleciendo."
    });
  }
}
