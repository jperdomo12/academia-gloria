...
import {
  ...
  BLOQUES_RESULTADO,
  PRACTICA_PROBLEMAS,
  FAMILIAS_PRUEBA_PROBLEMAS
} from "./problemas-nivel-a-data.js";
...
function renderPractica() {
  detenerLecturaActual();
  const reto=PRACTICA_PROBLEMAS[estado.practicaIndice];
  const porcentaje=((estado.practicaIndice+1)/PRACTICA_PROBLEMAS.length)*100;
  $("barraPractica").style.width=`${porcentaje}%`;
  $("estadoPractica").textContent=`Reto ${estado.practicaIndice+1} de ${PRACTICA_PROBLEMAS.length}`;
...
$("botonComprobar").addEventListener("click",()=>{
  ...
  if (correcta) {
    ...
    $("botonSiguiente").textContent=estado.practicaIndice===PRACTICA_PROBLEMAS.length-1
      ? "Ir a la prueba →"
      : "Siguiente reto →";
  } else {
    botonElegido?.classList.add("incorrecta");
    $("mensajePractica").textContent="Todavía no. Vuelve a la pregunta, revisa qué representa cada dato y prueba otra vez.";
    $("mensajePractica").className="message revisar";
  }
});

$("botonSiguiente").addEventListener("click",()=>{
  if (estado.practicaIndice<PRACTICA_PROBLEMAS.length-1) {
  ...
  if (modo===MODOS_SESION_ACADEMICA.APRENDIZAJE) {
    try {
      recientes=await obtenerVariantesRecientes({
        actividadId:PROBLEMAS_META.actividadId,
        maximoSesiones:3
      });
    } catch (error) {
      console.warn("No se pudo consultar el historial de variantes de Problemas Nivel A.",error);
    }
  }

  estado.preguntasPrueba=FAMILIAS_PRUEBA_PROBLEMAS.map(familia=>{
...
async function iniciarPrueba() {
  ...
  estado.tiempo.reiniciar("prueba-problemas-nivel-a");
...
function renderPreguntaPrueba() {
  ...
  $("bloquePrueba").textContent=bloqueResultado(pregunta.bloqueId)?.titulo || "Problemas";
...
function construirMapaResultados() {
  return BLOQUES_RESULTADO.map(bloque=>{
    ...
    let textoEstado="Vuelve a este paso del método y practica otra variante.";

    if (total && correctas===total) {
      estadoId="solido";
      tituloEstado="Se vio firme hoy";
      textoEstado="Las respuestas de hoy muestran una estrategia bien conectada.";
    } else if (correctas>0) {
      estadoId="camino";
      tituloEstado="En camino";
      textoEstado="Parte de la estrategia está presente; unas variantes más ayudarán a consolidarla.";
    }
...
async function finalizarPrueba() {
  ...
  if (reforzar.length) {
    $("textoCierre").textContent=`${nombreAlumno}, ya tenemos una fotografía útil: algunos pasos del método están firmes y otros conviene volver a practicar con calma.`;
  } else if (enCamino.length) {
    $("textoCierre").textContent=`${nombreAlumno}, la ruta para resolver problemas está apareciendo. Unas variantes más ayudarán a hacerla más automática.`;
  } else {
    $("textoCierre").textContent=`${nombreAlumno}, hoy comprender, planificar, calcular y comprobar se vieron firmes.`;
  }
  ...
  try {
    estado.guardado=await guardarSesionAcademica({
      modo,
      actividadId:PROBLEMAS_META.actividadId,
      tituloActividad:PROBLEMAS_META.tituloActividad,
      versionActividad:PROBLEMAS_META.version,
      cursoReferencia:PROBLEMAS_META.cursoReferencia,
      materia:PROBLEMAS_META.materia,
      tema:PROBLEMAS_META.tema,
      origen:"curso",
    ...
    console.error("No se pudo guardar la sesión de Problemas Nivel A.",error);
  ...
  if (typeof mostrarCelebracion==="function") {
    mostrarCelebracion({
      titulo:"¡Problema recorrido!",
      mensaje:"Ya sabemos qué pasos de resolución seguir fortaleciendo."
    });
  }
}
