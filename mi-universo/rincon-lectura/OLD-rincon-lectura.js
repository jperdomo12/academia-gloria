    ...
    $("readingHistory").innerHTML = sessions.map(session => `
      <article class="history-card">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:start">
          <div>
            <span style="font-weight:900;color:#7c3aed">${escapeHtml(session.categoria || "Lectura")}</span>
            <h3 style="margin:5px 0">${escapeHtml(session.titulo || "Historia")}</h3>
          </div>
          <span class="feedback correct" style="margin:0">${formatDuration(session.duracion || 0)}</span>
        </div>

        <p style="color:#64748b;font-weight:650">${escapeHtml(formatFirestoreDate(session.creadaEn))}</p>
        ...
        <button
          ...
          data-delete-story="${escapeHtml(session.historiaId || session.id)}"
          data-delete-title="${escapeHtml(session.titulo || "esta aventura")}"
        >
          🗑️ Eliminar aventura guardada
        </button>
      ...
      .forEach(button => {
        button.onclick = async () => {
          ...
          try {
            ...
            alert(
              `No se pudo eliminar la aventura.
${deleteError.message}`
            );
            button.disabled = false;
            button.textContent = "🗑️ Eliminar aventura guardada";
          }
        };
      });
  } catch (error) {
