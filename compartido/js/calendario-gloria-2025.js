/* Academia de Gloria Valentina | Mi Calendario 2025 SUPER PRO */
...
(() => {
  ...
  function renderCalendario() {
    ...
    MESES.forEach(([mes, nombre]) => {
      ...
      let celdas = DIAS.map((d) =>
        `<div class="gloria-dia-semana">${d}</div>`
      ).join("");

      celdas += '<div class="gloria-dia vacio"></div>'.repeat(primerDiaLunes(ANIO, mes));
  ...
  function eventosFiltrados() {
    ...
    if (mesSeleccionado !== "todos") {
      eventos = eventos.filter((e) => Number(e.fecha.slice(5, 7)) - 1 === Number(mesSeleccionado));
    }
  ...
  function renderEventos() {
    ...
    contenedor.querySelectorAll("[data-editar]").forEach((boton) => {
      boton.onclick = () => {
        const evento = leerEventos().find((e) => e.id === boton.dataset.editar);
        abrirModal(evento);
      };
    });
  ...
  function abrirModal(evento = null, fecha = "") {
    ...
    $("eventoFecha").value = evento?.fecha || fecha || `${ANIO}-01-01`;
  ...
  function guardarEvento(eventoFormulario) {
    ...
    if (indice >= 0) eventos[indice] = nuevo;
  ...
  function aplicarFiltroMes() {
    document.querySelectorAll(".gloria-mes").forEach((mes) => {
      mes.hidden = mesSeleccionado !== "todos" && mes.dataset.mes !== mesSeleccionado;
    });
  }
