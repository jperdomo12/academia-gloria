...
|---|---:|---|---|
| 1.5-rc1 | 23/09/2026 | Product Owner + AI Collaborator | Corrige una omisión real detectada en Matezonas: tres documentos nuevos no provocaban cambio. La normalización deja de eliminar indiscriminadamente toda la query de `iframe/embed/object`, conserva parámetros identificadores salvo en embeds de YouTube y añade una segunda detección de recursos Drive/Docs y archivos ofimáticos serializados dentro del HTML. Pendiente de validación en el navegador autenticado antes de promover a 1.5. |
...
- recursos Drive/Docs y archivos ofimáticos reconocibles que aparezcan serializados dentro del HTML aunque Google Sites no los exponga todavía como enlaces DOM normales.

Las URLs de recursos se normalizan para reducir falsos positivos derivados de parámetros técnicos variables.

La regla vigente desde 1.5-rc1 es:

- eliminar parámetros técnicos conocidos (`authuser`, `usp`, `pli`, `embed_config`, `embedded`, `rm`);
- ordenar los parámetros restantes para que el orden no produzca diferencias falsas;
- conservar parámetros que puedan identificar un documento;
- **solo** en embeds de YouTube/YouTube Education/YouTube NoCookie se elimina completamente la query, porque allí ya se comprobó que parámetros dinámicos provocaban ruido;
- complementar el DOM con URLs serializadas de `drive.google.com`, `docs.google.com` y recursos con extensiones `.pdf`, `.doc/.docx`, `.ppt/.pptx`, `.xls/.xlsx`.

Esta distinción evita que varios documentos embebidos diferentes se reduzcan accidentalmente a una misma URL normalizada.
...
El 23/09/2026 el Product Owner confirmó visualmente que **Matezonas contenía tres documentos nuevos**, mientras que el monitor 1.4 seguía indicando ausencia de cambios.

La revisión del algoritmo identificó dos puntos de pérdida de señal:

1. para `iframe/embed/object` se eliminaba toda la query string, lo que podía borrar parámetros usados para distinguir documentos;
2. el monitor solo recogía recursos expuestos como elementos DOM normales y podía omitir referencias serializadas dentro del HTML que Google Sites utiliza para carga dinámica.

La corrección 1.5-rc1 conserva la línea base `gaudem6_detalle_v2` y amplía la extracción. Al reutilizar una línea base creada con una versión anterior, la primera ejecución puede señalar algún espacio adicional si ya contenía recursos serializados que antes no se registraban. Esos casos deben revisarse antes de aceptar la nueva línea base.
